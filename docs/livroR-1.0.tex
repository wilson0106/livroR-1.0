...
Caso você ainda não tenha seus dados, vamos verificar como conduzir o teste-t no R por meio de exemplos utilizando valores fictícios. Vamos ensinar como criar valores aleatórios no R.
...
\NormalTok{text_tbl <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{Atributos =} \KeywordTok{c}\NormalTok{(}\StringTok{"Tipo de variável"}\NormalTok{,}
                                     \StringTok{"Quantidade de variáveis"}\NormalTok{,}
                                     \StringTok{"Hipótese nula"}\NormalTok{,}
                                     \StringTok{"Fórmula",}
\StringTok{                                     "}\NormalTok{Observação}\StringTok{"),}
\StringTok{                       Características = c("}\NormalTok{Quantitativa e categórica",}
                                           \StringTok{"2 (1 de cada tipo obrigatoriamente)"}\NormalTok{,}
                                           \StringTok{"A diferença na média da variável quantitativa dos grupos é igual a 0."}\NormalTok{,}
                                           \StringTok{"$$t=}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{frac\{}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_1-}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_2\}\{s_\{}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_1-}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_2\}\}$$, onde, $}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_1$: média do grupo 1, $}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{overline\{X\}_2$: média do grupo 2, s: erro padrão da diferença entre os dois grupos."}\NormalTok{,}
                                           \StringTok{"Não há a necessidade de post-hoc nem expressa-la graficamente."}\NormalTok{))}

\KeywordTok{kbl}\NormalTok{(text_tbl, }\DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{valign =} \StringTok{"c"}\NormalTok{, }\DataTypeTok{escape =}\NormalTok{ F, }\DataTypeTok{caption =} \StringTok{"Principais características do teste-t para duas amostras"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{full_width =}\NormalTok{ F, }\DataTypeTok{latex_options =} \StringTok{"striped"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{column_spec}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DataTypeTok{bold =}\NormalTok{ T, }\DataTypeTok{width =} \StringTok{"10em"}\NormalTok{) }\OperatorTok{%>%}
...
{\centering \includegraphics[width=0.7\linewidth]{livroR-1.0_files/figure-latex/testet-duas-dens-1} 

}

\caption{Curva de densidade representando o valor do teste-t bicaudal para a alteração do peso de gastrópodes em relação ao alimento a um intervalo de confiança de 95\%. O ponto azul indica o valor do teste.}\label{fig:testet-duas-dens}
\end{figure}
